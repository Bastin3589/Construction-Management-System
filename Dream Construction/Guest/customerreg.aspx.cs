    ...
    public partial class customerreg : System.Web.UI.Page
    {
        ...
        protected void register_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToInt32(ob1) == 0)
            {
                ...
                object ob = db.exescalar("insert into tbl_login values('" + username.Text + "','" + password.Text + "','customer','approved');select @@IDENTITY");
                db.Dbclose();
                if (ob.ToString() != null)
                {

                    DateTime dd = Convert.ToDateTime(DateTime.Now.Date);
                    string reg_date = dd.Date.ToString("MM-dd-yyyy");

                    int s = db.exenonquery("insert into tbl_customer values ('" + customername.Text + "','" + ob.ToString() + "','" + ddllocation.SelectedValue + "','" + customerpincode.Text + "','" + customerhousename.Text + "','" + customercontact.Text + "','" + customeremail.Text + "')");
                    if (s == 1)
                    {
                        string to = customeremail.Text; //To address      
